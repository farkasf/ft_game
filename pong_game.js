...
function startGameFunction(window, document, THREE) {
  ...
  function handleWinningCondition() {
    if (score.player1 === 1) {
      stopGame();
      renderWinningScreen(document.querySelector("#player1name").innerHTML);
    } else if (score.player2 === 1) {
      stopGame();
      renderWinningScreen(document.querySelector("#player2name").innerHTML);
    }
  }
  ...
  function initCamera() {
    if (gameMode === 1) {
      ...
      camera = new THREE.PerspectiveCamera(VIEW_ANGLE, ASPECT, NEAR, FAR);
      camera.position.set(FIELD_WIDTH, FIELD_LENGTH * 0.6, 0);
  ...
  function initBackgroundPlanes() {
    ...
    const textureRepeat =
      (Math.max(FIELD_WIDTH, FIELD_LENGTH) / planeSize) * 10;
  ...
  function movePaddleOne() {
    if (gameMode === 1) {
      ...
      paddle1.position.x = Math.min(
        ...
        Math.max(-halfFieldWidth + halfPaddleWidth, paddle1.position.x)
      );
    } else {
      // 1 vs 1
      switch (true) {
